...
export async function createUserApiUsage({
  userId,
  apiCallsLeft,
  monthlyLimit,
}: {
  ...
  try {
    return await db
      .insert(userApiUsage)
      .values({ userId, apiCallsLeft, monthlyLimit });
  } catch (error) {
    console.error("Failed to create userApiUsage API usage in database");
    throw error;
  }
}
...
export async function updateUserApiUsage({
  ...
  try {
    return await db
      .update(userApiUsage)
      .set({ apiCallsLeft, monthlyLimit })
      .where(eq(userApiUsage.userId, userId));
  } catch (error) {
    console.error("Failed to update user API usage in database");
    throw error;
  }
}
